import { AuditResults, WorkflowAuditResults, ContactAuditResults, CompanyAuditResults, UserAuditResults, DealAuditResults, LeadAuditResults, GlobalAuditResults } from "@/lib/audit/types";
 ...
 * EP-18 : Leads coefficient 1.0 (standard).
...
export function calculateGlobalScore(
  ...
  const leadScore = (leadResults?.hasLeads && !leadResults?.scopeError) ? leadResults.score : null;

  // Weighted scoring: deals get 1.5, leads get 1.0, all others get 1.0
  const DEAL_WEIGHT = 1.5;
  const LEAD_WEIGHT = 1.0;
  ...
  if (leadScore !== null) { weightedSum += leadScore * LEAD_WEIGHT; totalWeight += LEAD_WEIGHT; }
  ...
  const leadWeight = leadScore !== null ? LEAD_WEIGHT / totalWeight : 0;
